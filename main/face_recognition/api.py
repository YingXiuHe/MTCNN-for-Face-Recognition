...
def face_encodings(face_image, known_face_locations=None, num_jitters=6):
    """
...
def compare_faces(known_face_encodings, face_encoding_to_check, tolerance=0.6):
    ...
    scores = list()
    for i, dis in enumerate(list(face_distance(known_face_encodings, face_encoding_to_check))):
        scores.append(dis)
        min_dis = min(scores)
        if min_dis < tolerance:
            print('Matched:{}%'.format(float(1-min_dis+0.2)*100))
